...
## Known Issues
- Email отправляется через mock/SMTP по env — для продакшена задать `SMTP_HOST/User/Password`
- 2FA работает при входе, но нет «вспомнить устройство» (trusted devices)
- Нет полноценной модерации контента (скрытие заказов/услуг), только жалобы
- app.py всё ещё крупный (~1500 строк), при росте — дальнейшее разбиение или переход на фреймворк
