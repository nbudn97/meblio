...
- Нет «вспомнить устройство» как явного чекбокса (device-cookie выдаётся автоматически после TOTP)
- Модерация компаний/отзывов (is_hidden только у заказов/услуг)
- app.py ~1700 строк — дальнейший рост потребует нового разбиения
