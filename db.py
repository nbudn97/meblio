...
REGIONS = [
    ...
    "Свердловская область", "Новосибирская область", "Тюменская область",
    "Ростовская область", "Волгоградская область", "Самарская область",
    "Челябинская область", "Нижегородская область", "Воронежская область",
    "Пермский край", "Омская область", "Красноярский край",
    "Республика Татарстан", "Башкортостан", "Тульская область",
    "Калининградская область", "Иркутская область",
    ...
    "Алтайский край", "Забайкальский край", "Камчатский край", "Ставропольский край",
    "Хабаровский край", "Амурская область", "Архангельская область",
    "Астраханская область", "Белгородская область", "Брянская область",
    "Владимирская область", "Вологодская область", "Ивановская область",
    ...
    "Пензенская область", "Псковская область", "Рязанская область",
    "Сахалинская область", "Смоленская область", "Тамбовская область",
    "Тверская область", "Томская область", "Тюменская область (Югра)",
    ...
    "Ямало-Ненецкий автономный округ", "Республика Крым",
]
...
def init_db():
    ...
    with connect() as conn:
        ...
        for name in REGIONS:
            conn.execute("INSERT OR IGNORE INTO regions (name, slug) VALUES (?, ?)", (name, slugify(name)))
        for row in conn.execute("SELECT id, name FROM regions WHERE slug = '' OR slug IS NULL").fetchall():
            conn.execute("UPDATE regions SET slug = ? WHERE id = ?", (slugify(row["name"]), row["id"]))
...
def seed_articles(conn):
    admin = conn.execute("SELECT id FROM users WHERE email = 'admin@meblio.ru'").fetchone()
    author_id = admin["id"] if admin else None
    articles_data = [
        ("kak-vybrat-izgotovitelya-mebli",
         "Как выбрать производителя мебели: чек-лист для B2B-заказчика",
         "Компетенции, мощность, документы и примеры работ — на что смотреть при выборе подрядчика.",
         """## Проверьте компетенции

Убедитесь, что производитель работает с нужными материалами (ЛДСП, МДФ эмаль, шпон) и типами изделий.

## Запросите мощность

Спросите про месячный объём выпуска: «до 120 изделий в месяц» означает серийное производство, индивидуальные кухни — штучная работа.

## Посмотрите портфолио и сертификаты

На карточке компании изучите галерею выполненных работ и загрузленные сертификаты качества.

## Начните с чата

Обсудите детали в переписке: сроки, фурнитуру, доставку и монтаж. Хорошая коммуникация на старте — главный предиктор успеха сделки."""),
        ("ldsp-mdf-shpon-razlichiya",
         "ЛДСП, МДФ или шпон: что выбрать для проекта",
         "Сравниваем три основных материала корпусной мебели: цена, износостойкость, внешний вид.",
         """## ЛДСП — бюджет и скорость

Оптимален для офисной мебели и серийных партий. Цена от 1200 руб/м².

## МДФ эмаль — премиальный вид

Фасады сложных форм, глубокие цвета. От 4500 руб/м².

## Шпон — натуральность

Натуральный рисунок дерева, реставрируется. Требует бережного ухода.

## Что учесть в смете

Материал — обычно 30–40% бюджета заказа. Остальное: фурнитура, работа, доставка и монтаж."""),
        ("kak-sostavit-tz-na-mebel",
         "Как составить техзадание, чтобы получить точный расчёт",
         "Размеры, материалы, фурнитура, условия доставки: структура ТЗ, на которую подрядчики отвечают быстро.",
         """## Укажите габариты и количество

Точные размеры по каждому изделию и общее количество — без этого расчёт будет «плюс-минус».

## Перечислите материалы

ЛДСП Egger или МДФ эмаль? Какой цвет? Какая столешница?

## Опишите фурнитуру

Blum, Hettich или аналог — разница в цене фасада может достигать 20%.

## Приложите файлы

Чертежи, эскизы и фото референсов сокращают время согласования в разы."""),
    ]
    for slug, title, excerpt, body in articles_data:
        conn.execute(
            "INSERT OR IGNORE INTO articles (slug, title, excerpt, body_md, is_published, author_id, created_at, updated_at) VALUES (?, ?, ?, ?, 1, ?, ?, ?)",
            (slug, title, excerpt, body, author_id, now(), now()),
        )
